...
export const JumpUI: React.FC<JumpUIProps> = ({
  score,
  height,
  gameOver,
  isPaused,
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-gray-800 rounded-lg p-4 border border-neon-cyan">
          <div className="text-neon-cyan text-sm font-semibold mb-1">高度</div>
          <div className="text-3xl font-bold text-neon-lime">{Math.floor(height)}</div>
        </div>
      </div>
      ...
      {gameOver && (
        <div className="bg-gradient-to-r from-neon-lime to-neon-cyan p-6 rounded-lg text-center">
          ...
          <p className="text-black font-semibold mb-4">
            最终得分: {score} | 最高高度: {Math.floor(height)}
          </p>
          <button
            onClick={onRestart}
